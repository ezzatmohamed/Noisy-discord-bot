...
module.exports = {
    ...
    handler: async (message, args, session, bot, verbose=true, reply=true) => {
        ...
        const saved_queues = await player.getSavedQueues()
        
        if (session.db_list_message) {
            session.db_list_message.setContent({ ...session.db_list_message.params, component: undefined })
            await session.db_list_message.send()

            bot.removeListener('clickButton', session.db_list_click_button_listener)

            session.db_list_message = undefined
            session.db_list_click_button_listener = undefined
        }
        ...
        const db_list_message = new bot.MessagesController.Message(message.channel, {}, message)

        const update_db_list_message = async (db_list_message, page=-1) => {
            let { embed, num_pages, page_idx } = player.createSavedQueuesEmbed(saved_queues, 'Saved Queues', `(**${saved_queues.length}**) Saved Queues Found \n⠀\n`, page)
            ...
            if (embed)  {
                db_list_message.setContent({ embed, component: [buttons] })
            }
            else
                db_list_message.setContent({
                    type: 'danger',
                    description: `No saved queues exist`,
                })
            await db_list_message.send()
            return page_idx
        }

        page_idx = await update_db_list_message(db_list_message, page_idx)
        
        async function db_list_click_button_listener(button) {
        ...
        bot.on('clickButton', db_list_click_button_listener)

        session.db_list_message = db_list_message
        session.db_list_click_button_listener = db_list_click_button_listener
